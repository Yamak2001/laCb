...
class DPRNNModel(BaseSeparationModel):
    """Dual-Path RNN TasNet model for audio source separation"""
    ...
    def load_model(self) -> None:
        """Load a pre-trained DPRNN-TasNet model for voice separation."""
        start_time = time.time()
        max_retries = 3
        
        # Try multiple model sources in case one fails
        model_sources = [
            "mpariente/DPRNNTasNet_WHAM_sepclean",  # Standard Asteroid model name
            "JorisCos/ConvTasNet_Libri2Mix_sepnoisy_16k",    # Fallback to ConvTasNet
        ]
        ...
        for source in model_sources:
            for attempt in range(max_retries):
                try:
                    logger.info(f"Attempting to load DPRNN model from {source} (attempt {attempt+1}/{max_retries})")
                    model = DPRNNTasNet.from_pretrained(
                        source,
                        cache_dir=MODELS_DIR
                    )
                    logger.info("DPRNN model loaded successfully!")
        ...
        logger.warning("All model loading attempts failed. Creating fallback DPRNN model.")
        try:
            model = DPRNNTasNet(
                n_src=2,  # Number of sources to separate
                n_repeats=6,  # Number of repeats in DPRNN
                bn_chan=128,  # Number of bottleneck channels
                hid_size=128,  # Hidden size in RNN
                chunk_size=100,  # Chunk size in DPRNN
            ...
            logger.info("Created fallback DPRNN model")
            self.model = model
                    except Exception as e:
            logger.error(f"Failed to create fallback DPRNN model: {e}")
            raise
        def separate_sources(self, audio_path: str, output_dir: str) -> List[str]:
        """Separate audio sources from a mixed input file using the DPRNN-TasNet model."""
        ...
        try:
            ...
            logger.info("Processing audio with DPRNN model")
            ...
            logger.error(f"Error in DPRNN audio source separation: {e}")
            raise