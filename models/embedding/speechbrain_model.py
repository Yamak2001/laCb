...
class SpeechBrainModel(BaseEmbeddingModel):
    def __init__(self, model_name: str, use_gpu: bool = False, 
                 pretrained_model: str = "speechbrain/spkrec-xvect-voxceleb"):
        super().__init__(model_name, use_gpu)
        self.device = None
        self.model = None
        self.pretrained_model = pretrained_model
        self.embedding_dim = 512  # Default for X-Vector
    def load_model(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loading SpeechBrain X-Vector model from: {self.pretrained_model}")
            
            # Set device
            self.device = torch.device("cuda" if self.use_gpu and torch.cuda.is_available() else "cpu")
            device_name = "CUDA" if self.device.type == "cuda" else "CPU"
            logger.info(f"Using {device_name} for X-Vector model")
            
            # Load the pretrained model
            self.model = EncoderClassifier.from_hparams(
                source=self.pretrained_model,
                savedir=os.path.join(MODELS_DIR, "speechbrain_xvector"),
                run_opts={"device": self.device}
            )
            
            logger.info("SpeechBrain X-Vector model loaded successfully")
            ...
            logger.error(f"Failed to load SpeechBrain X-Vector model: {e}")
            raise
        ...
        try:
            ...
            # Load the audio file using torchaudio (which SpeechBrain models expect)
            import torchaudio
            signal, fs = torchaudio.load(audio_path)
            
            # SpeechBrain models might expect specific sample rate and mono audio
            ...
            # Extract the embedding using the loaded signal instead of just the path
            ...
            logger.error(f"Error creating X-Vector voice embedding: {e}")
